...
void meshGrdidCell::DumpTri(FILE* fp, double* vtx, unsigned int* triid )
{
...
int meshGrdidCell::OnTri( double* point, double* vtx, unsigned int* triid, double& dist, double* on_point  )
{
	std::vector<int>::iterator it = facet.begin();
...
void PointToNormal::MakeNormal(int numpoint, double* points, double* normal, int grid[3], int* errnum, int** errid )
{
	...
	*errid = NULL;
	...
	for ( int i = 0; i < numpoint; i++ ){
		...
		normal_vec[0] = dir1[2] * dir3[1] - dir1[1] * dir3[2];
		normal_vec[1] = dir1[0] * dir3[2] - dir1[2] * dir3[0];
		normal_vec[2] = dir1[1] * dir3[0] - dir1[0] * dir3[1];
		lnglng = normal_vec[0] * normal_vec[0] + normal_vec[1] * normal_vec[1] + normal_vec[2] * normal_vec[2];
		double lng = sqrt( lnglng );
		//lng = 1.0;

		normal[3*i  ] = normal_vec[0]/lng;
		normal[3*i+1] = normal_vec[1]/lng;
		normal[3*i+2] = normal_vec[2]/lng;
#else
		double	normal_vec[3];	/* 法線ベクトル			*/
		double	lnglng;		/* 距離の自乗			*/

		normal_vec[0] = on_point[0] - points[3*i  ];
		normal_vec[1] = on_point[1] - points[3*i+1];
		normal_vec[2] = on_point[2] - points[3*i+2];
