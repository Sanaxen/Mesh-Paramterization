...
public:
	...
	void Add(float vmin[3], float vmax[3], int fid){
		...
		facet.push_back( fid+1 );
	};

	void DumpTri(FILE* fp, double* vtx, unsigned int* triid );

	int OnTri( double* point, double* vtx, unsigned int* triid, double& dist, double* on_point  );
};
...
public:
	...
	meshGrdid(int vtxnum, int trinum, double* vtx, unsigned int* triid){
		Init(vtxnum, trinum, vtx, triid);
	};

	void Init(int vtxnum, int trinum, double* vtx, unsigned int* triid){
		vtxnum_ = vtxnum;
		trinum_ = trinum;
		vtx_ = vtx;
		triid_ = triid;
...
public:
	PointToNormal(int vtxnum, int trinum, double* vtx, unsigned int* triid){
